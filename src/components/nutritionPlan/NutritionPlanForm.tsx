...
const NutritionPlanForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`Generazione piano nutrizionale con modello: ${selectedModel.id}`);
      ...
      if (plan === 'Si è verificato un errore. Riprova più tardi o seleziona un altro modello.') {
        throw new Error('Errore nella generazione del piano nutrizionale');
      }
      
      setNutritionPlan(plan);
    } catch (err) {
      console.error('Errore nella generazione del piano nutrizionale:', err);
      setError('Impossibile generare il piano nutrizionale. Prova a selezionare un altro modello AI.');
    } finally {
